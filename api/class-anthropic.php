...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if ($action === 'toggle') {
        ...
        $data = json_read(API_MASTER_JSON_PROVIDERS);
        if (isset($data['providers'][$id])) {
            $data['providers'][$id]['is_active'] = $active;
            json_write(API_MASTER_JSON_PROVIDERS, $data);
    ...
    if ($action === 'clear_logs') {
        $log_file = __DIR__ . '/logs/api-master.log';
        $deleted = 0;
    ...
    if ($action === 'get_stats') {
        ...
        if ($type === 'learning' || $type === 'all') {
            $learning_file = __DIR__ . '/data/learning-data.json';
        ...
        if ($type === 'vector' || $type === 'all') {
            $vector_file = __DIR__ . '/data/vector-data.json';
...
// 1. PATHS & CONSTANTS
// ========================================================================
define('API_MASTER_VERSION', '4.0.0');
define('API_MASTER_CONFIG_DIR', __DIR__ . '/config');
define('API_MASTER_JSON_PROVIDERS', API_MASTER_CONFIG_DIR . '/providers.json');
...
function get_log_content($lines = 100) {
    $log_file = __DIR__ . '/logs/api-master.log';
    if (!file_exists($log_file)) return "Log dosyası bulunamadı.";
...
<head>
    ...
    <title>API Master v<?php echo API_MASTER_VERSION; ?> - TAM YÖNETİM MERKEZİ</title>
    <style>
...
<div class="container">
    <div class="header">
        <h1>🏛️ API Master v<?php echo API_MASTER_VERSION; ?> - TAM YÖNETİM MERKEZİ</h1>
    ...
    <div id="dashboard-panel" class="panel active">
        ...
        <div style="background:#0f0f23; padding:16px; border-radius:12px; margin-top:16px;">
            ...
            <p><strong>📁 Modül Dizini:</strong> <?php echo __DIR__; ?></p>
            <p><strong>📄 Config Dosyası:</strong> <?php echo API_MASTER_JSON_PROVIDERS; ?></p>
        ...
        <div class="provider-grid" id="provider-grid">
            ...
            <div class="provider-card <?php echo ($p['is_active'] ?? false) ? '' : 'inactive'; ?>" data-id="<?php echo $id; ?>" data-name="<?php echo strtolower($p['name'] ?? $id); ?>" data-active="<?php echo ($p['is_active'] ?? false) ? '1' : '0'; ?>">
                ...
                <div class="provider-actions">
                    ...
                    <button class="btn btn-settings" onclick="showSettingsModal('<?php echo $id; ?>', '<?php echo htmlspecialchars($p['name'] ?? $id); ?>', '<?php echo addslashes($p['api_key'] ?? ''); ?>', '<?php echo addslashes($p['base_url'] ?? ''); ?>')">⚙️ Ayarlar</button>
...
async function refreshLearningStats() {
    ...
    document.getElementById('learning-patterns-list').innerHTML = '<span style="color:#8b8b9e;">📋 Öğrenme patternleri için learning/ klasörü yapılandırılmalıdır.</span>';
}
...
async function refreshVectorStats() {
    ...
    document.getElementById('vector-data-list').innerHTML = '<span style="color:#8b8b9e;">📊 Vektör verileri için vector/ klasörü yapılandırılmalıdır.</span>';
}
...
async function refreshCacheStats() {
    ...
    document.getElementById('cache-list').innerHTML = '<span style="color:#8b8b9e;">🗂️ Cache klasörü: ' + window.location.pathname.replace('panel.php', 'cache') + '</span>';
}

async function refreshLogs() {
    document.getElementById('log-content').innerHTML = '📝 Loglar yükleniyor...<br><br><span style="color:#8b8b9e;">Log dosyası: logs/api-master.log</span>';
}
...
document.getElementById('system-settings-form')?.addEventListener('submit', async (e) => {
    ...
    resultDiv.innerHTML = '<span style="color:#667ea;">⏳ Kaydediliyor...</span>';
    setTimeout(() => {
        resultDiv.innerHTML = '<span style="color:#238636;">✅ Ayarlar kaydedildi (JSON config)</span>';
    }, 500);
});
